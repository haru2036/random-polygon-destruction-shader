...
*/
Shader "Custom/Polygon Destruction (Audiolink)"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			void geom (triangle appdata input[3], uint primitiveId : SV_PrimitiveID, inout TriangleStream<g2f> stream)
			{
				...
				fixed rawdestruction = clamp(((sin(_Time * 4 * primitiveId * _Speed) * 0.5) + 0.5), 0.01, 1.0);
				float alpressure = AudioLinkData( ALPASS_AUDIOLINK + int2( 0, 1) ).x;
				fixed destruction = rawdestruction * alpressure;
